...
contract LiquidationOperatorPart2 is IUniswapV2Callee {
    ...
    constructor() {
        ...
        uniswapV2Pair_WBTC_USDT = IUniswapV2Pair(uniswapV2Factory.getPair(address(WBTC), address(USDT))); // WBTC/USDT Pool3
    ...
    function uniswapV2Call(
        ...
        console.log("Collateral amount %s",WBTC.balanceOf(address(this)));
        

        uint repay_WBTC = getAmountIn(debtToCover, reserve_WBTC_Pool3, reserve_USDT_Pool3);
        console.log("Repay amount %s",repay_WBTC);
        WBTC.transfer(address(uniswapV2Pair_WBTC_USDT), repay_WBTC);

        
        
        console.log("Pool Liquidity: WBTC: %s, USDT: %s", reserve_WBTC_Pool3, reserve_USDT_Pool3);
