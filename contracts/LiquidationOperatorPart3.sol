...
contract LiquidationOperatorPart3 is IUniswapV2Callee {
    ...
    IUniswapV2Pair immutable uniswapV2Pair_WBTC_USDT; //WBTC/USDT Pool3
    IUniswapV2Pair immutable uniswapV2Pair_WETH_USDC; // Pool 4
    ...
    constructor() {
        ...
        uniswapV2Pair_WETH_USDT = IUniswapV2Pair(uniswapV2Factory.getPair(address(WETH), address(USDT))); // Pool 1
        uniswapV2Pair_WBTC_WETH = IUniswapV2Pair(uniswapV2Factory.getPair(address(WBTC), address(WETH))); // Pool 2
        uniswapV2Pair_WBTC_USDT = IUniswapV2Pair(uniswapV2Factory.getPair(address(WBTC), address(USDT))); // WBTC/USDT Pool 3
        uniswapV2Pair_WETH_USDC = IUniswapV2Pair(uniswapV2Factory.getPair(address(WETH), address(USDC))); // WETH/USDC Pool 4
    ...
    function operate() external {
        ...
        //convert back to ETH for profit grading
    ...
    function uniswapV2Call(
        ...
        console.log("Collateral amount %s",WETH.balanceOf(address(this)));
        
        uint256 repay_WETH = getAmountIn(debtToCover, reserve_WETH_Pool4, reserve_USDC_Pool4);
        console.log("Repay amount %s",repay_WETH);
        WETH.transfer(address(uniswapV2Pair_WETH_USDC), repay_WETH);

        console.log("Pool Liquidity: WETH: %s, USDC: %s", reserve_WETH_Pool4, reserve_USDC_Pool4);
